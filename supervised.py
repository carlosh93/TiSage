...
def main():
    ...
    if cfg.get('double_batch_size', True):
        cfg['batch_size'] *= 2
    ...
    nsample = cfg.get('n_upsampled')
    if nsample is None:
        nsample = n_upsampled.get(cfg['dataset'])
    if nsample is not None and nsample <= 0:
        nsample = None
    if rank == 0:
        if nsample is None:
            logger.info("No n_upsampled setting for dataset '%s'; using natural dataset size.", cfg['dataset'])
        else:
            logger.info("Upsampling labeled set to %s samples per epoch.", nsample)
    trainset = SemiDataset(
        cfg['dataset'], cfg['data_root'], 'train_l', cfg['crop_size'], args.labeled_id_path, nsample=nsample
    )
    valset = SemiDataset(
