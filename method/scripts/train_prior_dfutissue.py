#!/usr/bin/env python3
"""
MedSigLIP embeddings + linear/MLP classifier for DFUTissue (4 classes) — standalone experiment.

Same flow as example_dermfound.py but using MedSigLIP (PyTorch only, no TensorFlow):
extract image embeddings from superpixel crops on labeled DFUTissue images,
train a linear or small MLP (embed_dim → 4), then run the same pipeline on the val set
...
Usage (from repo root):
  python train_prior_dfutissue.py
  python train_prior_dfutissue.py --save-embeddings medsiglip_labeled.npz --save-classifier medsiglip_head.pt --epochs 30
  python train_prior_dfutissue.py --load-embeddings medsiglip_labeled.npz --load-classifier medsiglip_head.pt --visualize
"""
import argparse
import os
import sys
...
# DFUTissue: 4 classes, order = GT indices (same as example_medsiglip.py)
CLASS_NAMES = ["background", "fibrin", "granulation", "callus"]
DFUTISSUE_GT_CLASSES = CLASS_NAMES
NUM_CLASSES = 4
CROP_SIZE = (448, 448)
...
def load_gt_mask(path, target_size):
    """
    Load DFUTissue annotation. Returns (H, W) with values in {0,1,2,3} (and 255 if ignore).
...
def collect_labeled_embeddings(
    ...
    Returns (embeddings, labels) with shapes (N, D) and (N,) in {0,1,2,3}. D from model.
    ...
    for line in lines:
        ...
        for idx, lid in enumerate(kept_ids):
            ...
            if gt_region.size == 0:
                continue
            majority = int(np.bincount(gt_region.astype(int), minlength=4).argmax())
...
def build_classifier(head_type, embed_dim, num_classes=4, hidden=256):
    if head_type == "linear":
...
def visualize_one_val_sample(
    ...
    head_type,
    device,
    save_path="medsiglip_classifier_val_viz.png",
    n_segments=64,
    compactness=10.0,
    min_size=100,
    context_margin=2,
    ...
    cmap = ListedColormap(["#000000", "#e74c3c", "#2ecc71", "#f39c12"])
    fig, axes = plt.subplots(1, 3, figsize=(12, 4))
    axes[0].imshow(arr)
    axes[0].set_title("Image")
    axes[0].axis("off")
    axes[1].imshow(pred_map, cmap=cmap, vmin=0, vmax=3)
    axes[1].set_title("MedSigLIP + classifier pred")
    axes[1].axis("off")
    mask_show = np.where(gt == 255, 0, gt)
    axes[2].imshow(mask_show, cmap=cmap, vmin=0, vmax=3)
...
def main():
    parser = argparse.ArgumentParser(
        description="MedSigLIP embeddings + classifier on DFUTissue (standalone experiment)"
    )
    parser.add_argument("--data-root", default="data/DFUTissue", help="DFUTissue root")
    parser.add_argument("--labeled-txt", default="splits/dfutissue/fixed/labeled.txt",
                        help="Labeled image/mask pairs")
    parser.add_argument("--val-txt", default="splits/dfutissue/val.txt",
    ...
    parser.add_argument("--superpixel-preset", choices=["default", "fine", "finer"], default="default",
                        help="default: 64,100; fine: 128,50; finer: 200,40 (more segments for small fibrin)")
    ...
    parser.add_argument(
        "--save-classifier",
        default="method/checkpoints/pretrained/medsiglip_head_dfutissue.pt",
    ...
    parser.add_argument("--class-weights", choices=["none", "balanced"], default="balanced",
                        help="balanced: upweight rare classes (e.g. fibrin); none: uniform")
    parser.add_argument("--fibrin-weight", type=float, default=1.0,
                        help="Extra multiplier for fibrin class in loss (only if class-weights=balanced). Try 1.5 or 2.0.")
    ...
    if args.load_classifier:
        ...
        if args.class_weights == "balanced":
            ...
            if args.fibrin_weight != 1.0:
                class_weights[1] *= args.fibrin_weight  # fibrin = class 1
                print(f"Using balanced class weights + fibrin-weight={args.fibrin_weight}.")
            else:
    ...
    if mean_miou is not None:
        print("\n" + "=" * 60)
        print("MedSigLIP + classifier on DFUTissue validation set")
        print("=" * 60)
