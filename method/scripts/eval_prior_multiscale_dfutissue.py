#!/usr/bin/env python3
"""
Standalone check for multi-scale MedSigLIP classifier prior on DFUTissue.
...
Usage:
  python scripts/eval_prior_multiscale_dfutissue.py --classifier-path method/checkpoints/pretrained/medsiglip_head_dfutissue.pt
  python scripts/eval_prior_multiscale_dfutissue.py --classifier-path method/checkpoints/pretrained/medsiglip_head_dfutissue.pt --beta 0.7
"""
...
from train_prior_dfutissue import (
    CLASS_NAMES,
    NUM_CLASSES,
    CROP_SIZE,
    HAS_SKIMAGE,
...
def print_report(mode_name, metrics):
    print("\n" + "=" * 68)
    print(f"{mode_name} prior on DFUTissue validation set")
    print("=" * 68)
    print(f"Images evaluated: {metrics['n_images']}")
    print(f"Pixel accuracy (mean): {metrics['pixel_acc']:.2f}%")
    print(f"mIoU (4 classes): {metrics['mIoU']:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate single vs multi-scale MedSigLIP classifier priors on DFUTissue val"
    )
    parser.add_argument("--data-root", default="data/DFUTissue")
    parser.add_argument("--val-txt", default="splits/dfutissue/val.txt")
    parser.add_argument("--classifier-path", required=True, help="Path to saved classifier checkpoint (.pt)")
    ...
    parser.add_argument("--coarse-n-segments", type=int, default=64)
    parser.add_argument("--coarse-compactness", type=float, default=10.0)
    parser.add_argument("--coarse-min-size", type=int, default=100)
    parser.add_argument("--coarse-context-margin", type=int, default=2)

    # Fine superpixels
    parser.add_argument("--fine-n-segments", type=int, default=200)
...
def write_results_tsv(path, coarse_n, coarse_min, fine_n, fine_min, beta, results):
